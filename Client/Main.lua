...
local function handle_message(message)
	...
	if msg_type == "registered" then
		...
		IntegrationService.OnConnected:Fire(username, token, is_hidden, data.userId, data.admin, data.game, data.displayName)

		start_heartbeat()

	elseif msg_type == "chat" then
		IntegrationService.OnChatMessage:Fire(data.username, data.message, data.timestamp, data.userId, data.admin, data.game, data.displayName)
...
local function getAdminKey()
	local env = getEnv()
	local k = nil
	if type(env) == "table" then
		k = rawget(env, "NAChatAdminKey")
	end
	if k == nil and type(_G) == "table" then
		k = rawget(_G, "NAChatAdminKey")
	end
...
function IntegrationService.Init(custom_config)
	...
	if not send_message("register", payload) then
